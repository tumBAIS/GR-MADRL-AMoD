...
class Trainer:
    def __init__(self, env, args):
        ...
        if self.data_dir == "./data_5_small_zones": self.zones = 5
        elif self.data_dir == "./data_11_small_zones": self.zones = 11
        else: self.zones = 38
    ...
    def __call__(self):
        ...
        for i in range(count_test_dates):
            ...
            for j in range(self.episode_max_steps):
                ...
                no_total_requests += tf.math.count_nonzero(obs["requests_state"][:,4])
                
                obs = next_obs
            
            test_rewards.append(test_reward)

        pd.DataFrame({"test_rewards_greedy": test_rewards}, index=test_dates).to_csv(self.results_dir + f"/test_rewards_greedy_{self.zones}zones_{self.n_veh}veh.csv")
        with open(self.results_dir + f"/avg_test_reward_greedy_{self.zones}zones_{self.n_veh}veh.txt", 'w') as f:
            f.write(str(np.mean(test_rewards)))
        with open(self.results_dir + f"/ratio_accepted_requests_{self.zones}zones_{self.n_veh}veh.txt", 'w') as f:
            f.write(str((no_accepted_requests/no_total_requests).numpy()))
