...
class Critic(tf.keras.Model):
    ...
    @tf.function # Altered to accommodate different critic update
    def call(self, state, act, hvs, request_mask_s, baseline=False):        
        ...
        if baseline==True:
            requests_state = tf.tile(requests_state, [self.n_veh*self.n_req_max,1,1])
            vehicles_state = tf.tile(vehicles_state, [self.n_veh*self.n_req_max,1,1])
            hvs = tf.tile(hvs, [self.n_veh*self.n_req_max,1,1])
            request_mask_s = tf.tile(request_mask_s, [self.n_veh*self.n_req_max,1])
            misc_state = tf.tile(misc_state, [self.n_veh*self.n_req_max,1,1])
        
        # add action to request state: reject/assign (0/1)
        requests_input = tf.cast(tf.expand_dims(tf.where(act==-1, 0., 1.), axis=2), tf.float16)
        #print(requests_input)
        ...
        act = tf.one_hot(act, depth=self.n_veh, axis=-1, dtype=tf.float16) # transform action into longer tensor, usage in baseline possible
