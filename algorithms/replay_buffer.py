...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        if tf.math.count_nonzero(self.rews, dtype=tf.float32) == 0: # To avoid errors in the divisions
            nonzero_obs = tf.cast(1., dtype=tf.float32)
        else:
            nonzero_obs = tf.math.count_nonzero(self.rews, dtype=tf.float32)/tf.cast(self.size, dtype=tf.float32) # For mean over batch
        nonzero_obs = tf.where(tf.math.is_nan(nonzero_obs), 1., nonzero_obs) # Delete NaN values
        nonzero_obs = tf.where(nonzero_obs < 1., 1., nonzero_obs) # To avoid massive upscaling
        ...
        return obses, hvses, acts, rews, next_obses, next_hvses, nonzero_obs
